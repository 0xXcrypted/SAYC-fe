	...
	async function createWill(interval, address, id, recipent) {
		...
		try {
			...
			if (ethereum) {
				...
				const willControllerContract = new ethers.Contract(
					contractAddress,
					contractABI,
					signer
				); //abi provides detalis about contract
				...
				const willtxn = await willControllerContract.createWill(
					interval,
					address,
					id,
					recipent,
			...
			>
				...
				<Button
					onClick={async () =>
						createWill(
							interval,
							'0xD69cC7bC7f7eDBf9868Bfca51920E87Df91f21FE',
							data.id,
							reciever
						)
					}
